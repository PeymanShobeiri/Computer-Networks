...
mcast_t *multicast_init(char *mcast_addr, int sport, int rport)
{
    ...
    bzero(&m->my_addr, sizeof(m->my_addr));
...
{
    int cnt = sendto(m->sock, msg, msglen, 0,
...
void multicast_setup_recv(mcast_t *m)
{
    ...
    if (setsockopt(m->sock, IPPROTO_IP, IP_ADD_MEMBERSHIP,
                   &m->mreq, sizeof(m->mreq)) < 0) {
...
{
    int cnt = recvfrom(m->sock, buf, bufsize, 0,
                       (struct sockaddr*)&m->my_addr, &m->my_addrlen);
    if (cnt < 0) {
        perror("recvfrom");
        exit(1);
    }
